param(
    [string]$TargetHost = $(if ($env:SMOKE_SSH_HOST) { $env:SMOKE_SSH_HOST } else { '103.219.192.147' }),
    ...
    [string]$RemotePath = $(if ($env:SMOKE_REMOTE_PATH) { $env:SMOKE_REMOTE_PATH } else { '/opt/' }),
